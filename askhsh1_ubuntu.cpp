...
GLfloat gameColors[][3] = {{1, 0.91f, 0.79f}, {1, 0, 0}, {0, 0, 1}, {0, 0, 0}};
				...
				if (cubeX1 >= 0 && cubeX2 >= 0 && cubeY1 >= 0 && cubeY2 >= 0){
					if (cubeX1 < 15 && cubeX2 < 15 && cubeY1 < 15 && cubeY2 < 15){
						if ((gameMatrix[cubeX1][cubeY1] == ROCK and gameMatrix[cubeX2][cubeY2] == SCISSORS) or (gameMatrix[cubeX1][cubeY1] == PAPPER and gameMatrix[cubeX2][cubeY2] == ROCK) or (gameMatrix[cubeX1][cubeY1] == SCISSORS and gameMatrix[cubeX2][cubeY2] == PAPPER)){
							...
							gameMatrix[cubeX1][cubeY1] = BLACK;
							score += 2;
							remainingMoves --;
							display();
							return true;
	...
	if (button == GLUT_LEFT_BUTTON and state == GLUT_DOWN and gameHasStarted and remainingMoves > 0){
	    ...
	    printf("posY: %f\n", posY);
	 	...
	 	if (currentMousePresses == 0){
			cubeX1 = (int)((posX + 22)/3);
			cubeY1 = (int)((posY + 22)/3);
	 		printf("x1: %d y1: %d\n", cubeX1, cubeY1);
	 	}
	 	else if (currentMousePresses == 1){

			cubeX2 = (int)((posX + 22)/3);
			cubeY2 = (int)((posY + 22)/3);
			printf("x2: %d y2: %d\n", cubeX2, cubeY2);
	...
	for (int i = x - 1; i < x + 4; i ++){
		for (int j = y - 1; j < y + 2; j ++){
			if (j != y or (i < x and i > x + 2)){
				if (i >= 0 and i < 15){
					if ( j >= 0 and j < 15){
						if (gameMatrix[x][y] != BLACK and gameMatrix[i][j] != BLACK){
...
void checkForTriples()
{
	for (int i = 0; i < 15; i ++){
		for (int j = 0; j < 15; j ++){
			if (i < 12){
				if (gameMatrix[i + 1][j] == gameMatrix[i][j] and gameMatrix[i + 2][j] == gameMatrix[i][j] and gameMatrix[i][j] != BLACK and gameMatrix[i + 1][j] != BLACK and gameMatrix[i + 2][j] != BLACK){
					//change score
					score += 10;
					//destroy neighbours
					...
					//destroy triplete
					gameMatrix[i][j] = BLACK;
					gameMatrix[i + 1][j] = BLACK;
					gameMatrix[i + 2][j] = BLACK;
					display();
				}
			}
			if (j < 12){
				if (gameMatrix[i][j + 1] == gameMatrix[i][j] and gameMatrix[i][j + 2] == gameMatrix[i][j] and gameMatrix[i][j] != BLACK and gameMatrix[i][j + 1] != BLACK and gameMatrix[i][j + 2] != BLACK){
					//change score
					score += 10;
					//destroy neighbours
					...
					//destroy triplete
					gameMatrix[i][j] = BLACK;
					gameMatrix[i][j + 1] = BLACK;
					gameMatrix[i][j + 2] = BLACK;
					display();
				}
			}
		}
...
void display()
{
	...
	for (int i = 0; i < 45; i+=3){
		for (int j = 0; j < 45; j+=3){
			glPushMatrix();
				glTranslatef(-21 + i, -21 + j, -70);
				if((gameMatrix[i/3][j/3] != BLUE) && (gameMatrix[i/3][j/3] != RED) && (gameMatrix[i/3][j/3] != BEIGE) && (gameMatrix[i/3][j/3] != BLACK))
				{
					glColor3f(1, 1, 1);
...
void init()
{
	...
	GLfloat light_position[] = { 0, 0, 3, -5.0 };
	glClearColor (0.0, 0.0, 0.0, 0.0);
